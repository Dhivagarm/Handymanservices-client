...
export default class profile extends Component {
    ...
    async componentDidMount(){
        ...
        axios.get(REACT_APP_URL+`/users/${resp.data._id}/avatar`)
        .then(
            response=>{
                this.setState({imgurl : REACT_APP_URL+`/users/${resp.data._id}/avatar`}) ;
            }
        )
        .catch(
            error=>{
    ...
    async handleDelAccount(event){
        ...
        const resp = await axios.delete(REACT_APP_URL+'/users/me',{
            headers: {
                'Authorization': localStorage.getItem('token')
              }
        })
    ...
    handleFileUpload(event){
        ...
        axios.post(REACT_APP_URL+'/users/me/avatar',
            data,
            {
                headers : {
                    'Authorization' : localStorage.getItem('token')
    ...
    handleDelTask(event){
        ...
        axios.delete(REACT_APP_URL+`/tasks/${this.state.id}`,{
            headers:{
                'Authorization' : localStorage.getItem('token')
            }
        })
